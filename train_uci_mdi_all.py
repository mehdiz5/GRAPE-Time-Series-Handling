...
def main():
    ...
    parser.add_argument('--split_by', type=str, default='random') # 'y', 'random'
    ...
    ## new
    for args.data in ['concrete', 'energy', 'housing', 'power',
                    'air', 'absentee']:
    # for args.data in ['concrete', 'energy', 'housing', 'wine', 'yacht']:
    # for args.data in [ 'kin8nm', 'naval', 'power', 'protein']:
        data = load_data(args)

        log_path = './uci/mdi_results/results/gnn_mdi_{}/{}/{}/'.format(args.time_steps, args.time_attr, args.data)
